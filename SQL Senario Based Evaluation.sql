...
Select account_master.account_number,customer_master.CUSTOMER_NUMBER,customer_master.FIRSTNAME,customer_master.lastname,account_master.account_opening_date
FROM account_master
inner join customer_master on account_master.customer_number=customer_master.CUSTOMER_NUMBER;

SELECT account_master.customer_number, customer_master.FIRSTNAME, account_master.account_number
FROM account_master, customer_master
WHERE account_master.customer_number = customer_master.CUSTOMER_NUMBER
AND day(account_opening_date) > 15
order by customer_number,account_number;

SELECT transaction_type as Transaction_Type,count(transaction_number) as Trans_Count
...
SELECT td.account_number, opening_balance+sum(transaction_amount) as Deposit_Amount
FROM account_master am INNER JOIN transaction_details td
ON am.account_number=td.account_number
...
select FIRSTNAME FROM customer_master cm 
